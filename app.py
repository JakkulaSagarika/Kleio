...
# AI Personal Stylist
    ...
    with col1:
        if st.button("Business Casual Outfits"):
            ...
            st.session_state.chat_history.append({"role": "assistant",
                                                  "content": "For business casual, try: Navy blazer + white button-up + dark wash jeans + loafers. Or: Midi skirt + blouse + cardigan + ankle boots. These looks are professional yet approachable!"})
            st.rerun()
    ...
    with col2:
        if st.button("Date Night Looks"):
            ...
            st.session_state.chat_history.append({"role": "assistant",
                                                  "content": "For date night: Little black dress + statement jewelry + heels, or high-waisted trousers + silk camisole + blazer + ankle boots. Choose comfortable yet elegant pieces!"})
            st.rerun()
    ...
    with col3:
        if st.button("Weekend Casual"):
            ...
            st.session_state.chat_history.append({"role": "assistant",
                                                  "content": "Weekend vibes: High-waisted jeans + crop top + denim jacket + white sneakers, or maxi dress + sandals + sun hat. Comfort meets style!"})
            st.rerun()
...
# Fashion Swap Hub
    ...
    with tab1:
        ...
        # Sample swap items
        swap_items = [
            {"user": "Sarah_M", "item": "Vintage Leather Jacket", "size": "M", "condition": "Like New",
             "looking_for": "Denim Jacket"},
            {"user": "Emily_K", "item": "Designer Handbag", "size": "One Size", "condition": "Good",
             "looking_for": "Crossbody Bag"},
            {"user": "Alex_R", "item": "Wool Sweater", "size": "L", "condition": "Excellent",
             "looking_for": "Cardigan"},
            {"user": "Maya_P", "item": "Silk Scarf", "size": "One Size", "condition": "New",
             "looking_for": "Statement Necklace"}
        ]
...
# Wardrobe Care Assistant
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🧼 Care Guide", "📅 Maintenance Schedule", "🔧 Quick Fixes", "📊 Wardrobe Analytics"])
